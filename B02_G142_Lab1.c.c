	...
	while (true){
		...
		switch(state){
			...
			case 1:
				...
				else if(button2_pushed && getMotorEncoder(motor1) > 3000){
					motor[motor1] = 0;
					state = state + 1;
					button1_pushed = false;
					resetMotorEncoder(motor1);
			...
			case 2:
				...
				else if(button1_pushed && getMotorEncoder(motor1) < -3000){// test this function to see if it will wait for it to turn 3000 ticks then change states
					motor[motor1] = 0;
					state = state - 1;
					button2_pushed = false;
					resetMotorEncoder(motor1);
